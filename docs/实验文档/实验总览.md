...
| 结果汇总 | [实验结果详述](实验结果详述.md) | 378 runs + serving | 跨策略对比、Oracle Gap、EMO 指标、控制器规则 |
...
**E2E 各策略节省总览**：九种策略在三个模型上的 E/tok 与功率节省对比。Pareto（Ours）是唯一在两类指标上都稳定靠前的策略——功率节省接近专用功耗策略（7B 上 -18.6% vs Dynamic），E/tok 损失控制在 1% 以内；dynamic / maxn 基线在两个维度均明显落后。

<img src="../../figures/e2e_benchmark/e2e_per_workload_savings.png" width="760" alt="按 workload 的节省分布">

**按 workload 拆解**：workload-aware 选择的实际价值所在——7B 上 Pareto 为 5 个 workload 选择了 612~1300 MHz 之间互不相同的 cap：长输出负载拿到接近单目标策略的节省，短输出负载避免了过度降频带来的延迟损失。固定频率策略无法同时覆盖两类负载。

<img src="../../figures/pareto_frontier/pareto_2d_ept_vs_tpot.png" width="520" alt="Pareto 前沿">

**多目标 Pareto 前沿**（E/tok × TPOT，对数轴）：每个 workload 一条前沿，选择器返回整条权衡面而非单点——knee point 给出自动折中，SLO 约束取满足延迟要求下的最省能点，alpha 旋钮可在前沿上连续滑动。8B 的前沿明显更短，对应其 memory-bound、频率不敏感的特征。

<img src="../../figures/phase13_analysis/oracle_gap.png" width="700" alt="Oracle gap">

**Oracle Gap**：以 lock-mode 全频率数据构造的 oracle 理论最优为标尺，衡量选择器的剩余优化空间。Pareto 距上界平均仅 +4.9%（8B 仅 +0.7%），而 dynamic 基线尚余 8.6%~23.4%。结论：单请求 E/tok 优化已接近天花板，收益重心转向长期运行的功率与热管理。

<img src="../../figures/phase13_analysis/multi_obj_hypervolume_comparison.png" width="700" alt="Hypervolume 对比">

**Hypervolume 对比**（支配的目标空间体积）：Pareto 在 8B / 14B 上领先第二名策略 21~31 倍——其解集均匀覆盖整条前沿；其他策略的点集高度聚集，HV 接近于零。这是"返回权衡面"相对"返回单点"的结构性优势。

> 完整图表见 [figures/](../../figures/) 各子目录；数值结论汇总见主 [README](../../README.md) 的"关键实验结果"一节。
